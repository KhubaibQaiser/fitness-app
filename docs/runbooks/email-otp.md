...
`OTP_PEPPER` may also be omitted locally (a fixed non-production fallback is used).

## Staging / production

1. Create a free [Resend](https://resend.com) account.
2. Verify a sending domain (SPF/DKIM/DMARC via Resend).
3. Set on the API host:

```bash
RESEND_API_KEY=re_...
EMAIL_FROM=GymOS <noreply@yourdomain.com>
OTP_PEPPER=<openssl rand -hex 32>
NODE_ENV=production
```

Production boot fails closed if `RESEND_API_KEY` or `OTP_PEPPER` is missing.
